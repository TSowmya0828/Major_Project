...
class ObjectDetector:
    ...
    def __init__(self, model_pth, conf_threshold):
    ...
    def detect(self, frame):
        ...
        # Inference
        hm, offset, whl = self.model(input_tensor.float().unsqueeze(0))

        hm = torch.sigmoid(hm)

        hm = hm.detach().numpy().squeeze(0).squeeze(0)
        offset = offset.detach().numpy().squeeze(0)
        whl = whl.detach().numpy().squeeze(0)
